...
@section('title', 'Modifier devise - ' . $currency->code)
...
<div class="max-w-2xl mx-auto pb-24 md:pb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 px-4 md:px-0">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                <a href="{{ route('admin.currencies.index') }}" class="text-neutral-500 hover:text-neutral-900 transition-colors">Devises</a>
                <span class="text-neutral-400">/</span>
                <span class="text-neutral-900 font-medium text-sm md:text-base">Modifier</span>
            </div>
            <h1 class="text-xl md:text-2xl font-serif font-bold text-neutral-900">{{ $currency->code }}</h1>
            <p class="text-xs md:text-sm text-neutral-500 mt-1">Modifiez les informations de la devise</p>
        </div>
        ...
        <div class="hidden md:flex items-center gap-3">
            ...
            <button type="submit" form="edit-currency-form" 
    ...
    <form id="edit-currency-form" action="{{ route('admin.currencies.update', $currency) }}" method="POST" class="space-y-6 px-4 md:px-0">
        @csrf
        @method('PUT')
            ...
            <div class="p-4 md:p-6 space-y-4 md:space-y-5">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="number" name="rate" value="{{ old('rate', $currency->rate) }}" step="0.0001" min="0.0001" required
            ...
            <div class="p-4 md:p-6 space-y-4">
                <div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_default" value="1" class="sr-only peer" {{ old('is_default', $currency->is_default) ? 'checked' : '' }}>
                    ...
                    @if($currency->is_default)
                        <p class="text-[11px] text-amber-600 mt-1 ml-14">⚠️ Si vous décochez cette option, une autre devise sera définie comme par défaut</p>
                    @endif
                </div>
                
                <div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $currency->is_active) ? 'checked' : '' }} {{ $currency->is_default ? 'disabled' : '' }}>
                        <div class="w-11 h-6 bg-neutral-300 rounded-full peer peer-checked:bg-gold peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all {{ $currency->is_default ? 'opacity-50 cursor-not-allowed' : '' }}"></div>
                    ...
                    @if($currency->is_default)
                        <p class="text-[11px] text-amber-600 mt-1 ml-14">⚠️ La devise par défaut ne peut pas être désactivée</p>
                    @endif
                </div>
            </div>
        </div>
