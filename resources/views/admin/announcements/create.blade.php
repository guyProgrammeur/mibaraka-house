...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-neutral-900 font-medium text-sm md:text-base">Nouvelle annonce</span>
            </div>
            <h1 class="text-xl md:text-2xl font-serif font-bold text-neutral-900">Créer une annonce</h1>
            <p class="text-xs md:text-sm text-neutral-500 mt-1">Ajoutez une nouvelle annonce promotionnelle ou information</p>
        </div>
        ...
        <div class="hidden md:flex items-center gap-3">
            ...
            <button type="submit" form="create-announcement-form" 
    ...
    <!-- Formulaire -->
    <form id="create-announcement-form" action="{{ route('admin.announcements.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 md:space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm">
            <div class="px-4 py-3 md:px-6 md:py-4 border-b border-neutral-100 bg-neutral-50/50">
                ...
                <p class="text-[11px] md:text-xs text-neutral-500 mt-0.5">Les informations de base de l'annonce</p>
            </div>
            <div class="p-4 md:p-6 space-y-4 md:space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required
                           class="w-full border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-colors @error('title') border-red-500 @enderror"
                           placeholder="Ex: Livraison Gratuite">
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Titre principal de l'annonce</p>
                    @error('title')
                ...
                <div>
                    ...
                    <textarea name="message" rows="3" 
                              class="w-full border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-colors @error('message') border-red-500 @enderror"
                              placeholder="Contenu de l'annonce...">{{ old('message') }}</textarea>
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Description ou message supplémentaire</p>
                    @error('message')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="badge" value="{{ old('badge') }}"
                           class="w-full border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-colors @error('badge') border-red-500 @enderror"
                           placeholder="Ex: NOUVEAU, PROMO, LIMITÉ...">
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Petit texte d'accroche (optionnel)</p>
                    @error('badge')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm">
            <div class="px-4 py-3 md:px-6 md:py-4 border-b border-neutral-100 bg-neutral-50/50">
                <h2 class="font-semibold text-sm md:text-base text-neutral-900">Type et affichage</h2>
                <p class="text-[11px] md:text-xs text-neutral-500 mt-0.5">Personnalisez l'apparence et le comportement</p>
            </div>
            <div class="p-4 md:p-6 space-y-4 md:space-y-5">
                <!-- Type -->
                <div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <select name="icon" class="flex-1 border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold transition-colors @error('icon') border-red-500 @enderror">
                    ...
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Icône Font Awesome pour illustrer l'annonce</p>
                    @error('icon')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', 0) }}"
                           class="w-32 border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold transition-colors @error('order') border-red-500 @enderror"
                           min="0">
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Plus le chiffre est petit, plus l'annonce apparaît en haut</p>
                    @error('order')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm">
            <div class="px-4 py-3 md:px-6 md:py-4 border-b border-neutral-100 bg-neutral-50/50">
                <h2 class="font-semibold text-sm md:text-base text-neutral-900">Bouton d'appel à l'action</h2>
                <p class="text-[11px] md:text-xs text-neutral-500 mt-0.5">Optionnel - Ajoutez un bouton à votre annonce</p>
            </div>
            <div class="p-4 md:p-6 space-y-4 md:space-y-5">
                <!-- Texte du bouton -->
                <div>
                    ...
                    <input type="text" name="button_text" value="{{ old('button_text') }}"
                           class="w-full border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold transition-colors @error('button_text') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <select name="button_link" class="w-full border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold transition-colors @error('button_link') border-red-500 @enderror">
                        @foreach($linkOptions as $value => $label)
                            <option value="{{ $value }}" {{ old('button_link') == $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Ou entrez une URL personnalisée ci-dessous</p>
                    @error('button_link')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="custom_url" value="{{ old('custom_url') }}"
                           class="w-full border border-neutral-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-gold transition-colors"
                           placeholder="https://exemple.com/page">
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Saisissez une URL complète pour remplacer la sélection ci-dessus</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm">
            <div class="px-4 py-3 md:px-6 md:py-4 border-b border-neutral-100 bg-neutral-50/50">
                <h2 class="font-semibold text-sm md:text-base text-neutral-900">Image</h2>
                <p class="text-[11px] md:text-xs text-neutral-500 mt-0.5">Optionnel - Ajoutez une image à votre annonce</p>
            </div>
            <div class="p-4 md:p-6">
                <div>
                    ...
                    <p class="text-[11px] md:text-xs text-neutral-400 mt-1">Format JPG, PNG, GIF, WEBP. Max 2MB.</p>
                    @error('image')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm">
            <div class="px-4 py-3 md:px-6 md:py-4 border-b border-neutral-100 bg-neutral-50/50">
                <h2 class="font-semibold text-sm md:text-base text-neutral-900">Période de validité</h2>
                <p class="text-[11px] md:text-xs text-neutral-500 mt-0.5">Optionnel - Limitez l'affichage dans le temps</p>
            </div>
            <div class="p-4 md:p-6">
                ...
                <p class="text-[11px] md:text-xs text-neutral-400 mt-2">Laissez vide pour une validité permanente</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-neutral-200 overflow-hidden shadow-sm">
            ...
            <div class="p-4 md:p-6">
                ...
                <p class="text-[11px] md:text-xs text-neutral-500 mt-2">Les annonces inactives ne seront pas affichées sur le site.</p>
            </div>
        </div>
    </form>
