...
<div class="max-w-4xl mx-auto pb-24 md:pb-6">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 px-4 md:px-0">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-neutral-900 font-medium text-sm md:text-base">Nouveau QR Code</span>
            </div>
            <h1 class="text-xl md:text-2xl font-serif font-bold text-neutral-900">Créer un QR Code</h1>
            <p class="text-xs md:text-sm text-neutral-500 mt-1">Créez un QR code personnalisé pour votre catalogue, catégorie ou produit</p>
        </div>
        ...
        <div class="hidden md:flex items-center gap-3">
            ...
            <button type="submit" form="create-qr-form" 
    ...
    <form id="create-qr-form" action="{{ route('admin.qr-codes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4 md:space-y-6 px-4 md:px-0">
        @csrf
            ...
            <div class="p-4 md:p-6 space-y-4 md:space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                        <label class="flex sm:flex-col items-center sm:text-center gap-3 sm:gap-2 border border-neutral-200 rounded-xl p-3.5 cursor-pointer hover:bg-neutral-50 transition-all type-option has-[:checked]:border-gold has-[:checked]:bg-gold/5" data-type="catalog">
                            <input type="radio" name="type" value="catalog" class="sr-only type-radio" {{ old('type', 'catalog') == 'catalog' ? 'checked' : '' }}>
                        ...
                        <label class="flex sm:flex-col items-center sm:text-center gap-3 sm:gap-2 border border-neutral-200 rounded-xl p-3.5 cursor-pointer hover:bg-neutral-50 transition-all type-option has-[:checked]:border-gold has-[:checked]:bg-gold/5" data-type="category">
                            <input type="radio" name="type" value="category" class="sr-only type-radio" {{ old('type') == 'category' ? 'checked' : '' }}>
                        ...
                        <label class="flex sm:flex-col items-center sm:text-center gap-3 sm:gap-2 border border-neutral-200 rounded-xl p-3.5 cursor-pointer hover:bg-neutral-50 transition-all type-option has-[:checked]:border-gold has-[:checked]:bg-gold/5" data-type="product">
                            <input type="radio" name="type" value="product" class="sr-only type-radio" {{ old('type') == 'product' ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" 
                              ...
                              placeholder="Description optionnelle du QR code...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <label class="flex items-center justify-center p-3 border border-neutral-300 rounded-lg cursor-pointer transition-all h-11 has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="size" value="small" {{ old('size', 'medium') == 'small' ? 'checked' : '' }} class="sr-only peer">
                            <span class="text-sm font-medium peer-checked:text-neutral-900 text-neutral-600">Petit (200px)</span>
                        </label>
                        <label class="flex items-center justify-center p-3 border border-neutral-300 rounded-lg cursor-pointer transition-all h-11 has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="size" value="medium" {{ old('size', 'medium') == 'medium' ? 'checked' : '' }} class="sr-only peer">
                            <span class="text-sm font-medium peer-checked:text-neutral-900 text-neutral-600">Moyen (300px)</span>
                        </label>
                        <label class="flex items-center justify-center p-3 border border-neutral-300 rounded-lg cursor-pointer transition-all h-11 has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="size" value="large" {{ old('size') == 'large' ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="p-4 md:p-6 space-y-4 md:space-y-5">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-5 gap-2">
                        <label class="flex flex-col items-center p-2 border border-neutral-300 rounded-lg cursor-pointer transition-all has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="poster_template" value="classic" {{ old('poster_template', 'classic') == 'classic' ? 'checked' : '' }} class="sr-only peer">
                            <span class="text-xs font-medium peer-checked:text-gold">Classique</span>
                        </label>
                        <label class="flex flex-col items-center p-2 border border-neutral-300 rounded-lg cursor-pointer transition-all has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="poster_template" value="luxury" {{ old('poster_template') == 'luxury' ? 'checked' : '' }} class="sr-only peer">
                            <span class="text-xs font-medium peer-checked:text-gold">Luxe</span>
                        </label>
                        <label class="flex flex-col items-center p-2 border border-neutral-300 rounded-lg cursor-pointer transition-all has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="poster_template" value="elegant" {{ old('poster_template') == 'elegant' ? 'checked' : '' }} class="sr-only peer">
                            <span class="text-xs font-medium peer-checked:text-gold">Élégant</span>
                        </label>
                        <label class="flex flex-col items-center p-2 border border-neutral-300 rounded-lg cursor-pointer transition-all has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="poster_template" value="modern" {{ old('poster_template') == 'modern' ? 'checked' : '' }} class="sr-only peer">
                            <span class="text-xs font-medium peer-checked:text-gold">Moderne</span>
                        </label>
                        <label class="flex flex-col items-center p-2 border border-neutral-300 rounded-lg cursor-pointer transition-all has-[:checked]:border-gold has-[:checked]:bg-gold/5">
                            <input type="radio" name="poster_template" value="minimal" {{ old('poster_template') == 'minimal' ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" name="poster_primary_color" value="{{ old('poster_primary_color', '#D4AF37') }}" 
                                   class="w-12 h-11 border border-neutral-300 rounded-lg cursor-pointer">
                            <input type="text" name="poster_primary_color_hex" value="{{ old('poster_primary_color', '#D4AF37') }}" 
                                   class="flex-1 border border-neutral-300 rounded-lg px-3 h-11 focus:outline-none focus:border-gold transition-colors font-mono text-sm"
                                   placeholder="#D4AF37">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" name="poster_background_color" value="{{ old('poster_background_color', '#FFFFFF') }}" 
                                   class="w-12 h-11 border border-neutral-300 rounded-lg cursor-pointer">
                            <input type="text" name="poster_background_color_hex" value="{{ old('poster_background_color', '#FFFFFF') }}" 
                                   class="flex-1 border border-neutral-300 rounded-lg px-3 h-11 focus:outline-none focus:border-gold transition-colors font-mono text-sm"
                                   placeholder="#FFFFFF">
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="color" name="poster_text_color" value="{{ old('poster_text_color', '#1a1a1a') }}" 
                                   class="w-12 h-11 border border-neutral-300 rounded-lg cursor-pointer">
                            <input type="text" name="poster_text_color_hex" value="{{ old('poster_text_color', '#1a1a1a') }}" 
                                   class="flex-1 border border-neutral-300 rounded-lg px-3 h-11 focus:outline-none focus:border-gold transition-colors font-mono text-sm"
                                   placeholder="#1a1a1a">
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="show_brand_name" value="1" class="rounded border-neutral-300 text-gold focus:ring-gold" {{ old('show_brand_name', true) ? 'checked' : '' }}>
                        <span class="text-sm text-neutral-700">Afficher le nom de la marque</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="show_tagline" value="1" class="rounded border-neutral-300 text-gold focus:ring-gold" {{ old('show_tagline', true) ? 'checked' : '' }}>
