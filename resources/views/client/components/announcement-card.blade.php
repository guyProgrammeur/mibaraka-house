@if(isset($announcement))
    ...
    @elseif($announcement->type == 'button')
        <div class="bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg p-4 text-white mb-4">
            <div class="flex items-center justify-between flex-wrap gap-3">
                ...
                @if($announcement->button_text && $announcement->button_link)
                    <a href="{{ $announcement->button_link }}" class="bg-white text-blue-600 px-4 py-2 rounded-lg font-semibold hover:bg-gray-100 transition shadow-md">
    ...
    @elseif($announcement->type == 'image')
        @if($announcement->image_url)
            <a href="{{ $announcement->button_link ?: '#' }}" class="block mb-4 transition-transform hover:scale-[1.02] duration-300">
                <img src="{{ $announcement->image_url }}" alt="{{ $announcement->title }}" class="w-full rounded-lg shadow-md">
            </a>
        @endif
        
    @elseif($announcement->type == 'image_text')
        <div class="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-100 mb-4 hover:shadow-xl transition-shadow duration-300">
    ...
    @elseif($announcement->type == 'banner')
        <div class="bg-gradient-to-r from-purple-600 to-pink-600 rounded-xl p-6 text-white text-center mb-4 shadow-lg">
            @if($announcement->icon)
